...
class Crawler:
    def __init__(self, keyword=None, writer=None):
        ...
        print(' - Cralwer.__init__()')

    def crawl(self):
        html_s = time.time()
        self.get_html()  # self.html 셋팅
        html_e = time.time()
        print('get_html() time: ', html_e - html_s)
        ...
        print(' - start check_duplacate()', time.time() - html_s)
    ...
    def get_html(self):
        ...
        try:
            if self.keyword:  # 키워드로 검색할 때는 최신순으로 정렬된 페이지를 크롤링
                ...
                print(f' - start driver.get({url})')
                self.driver.get(url)
                time.sleep(2)
                print(' - start driver.find_element')
    ...
    def get_article_txid_for_detail(self):
        print(' - get_article_txid_for_detail()')
    ...
    def check_duplicate(self):
        ...
        print(' - self.article_txid = ', self.article_txid_list)
        conn = self.connect
        cur = conn.cursor()
    ...
    @property
    def connect(self):
        db = json.load(open(os.getcwd() + '/db_info.json'))
